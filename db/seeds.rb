...
#can scrape PCGS population from here ($5 gold example) https://www.pcgs.com/pop/detail/category/61?l=liberty-head-5-1839-1908&ccid=0&t=5&p=MS&pn=1&ps=-1
...
def create_coin_series(page, category, designer, denomination)
  Coin.transaction do
    page.css('tbody tr').each do | tr | 
      pcgs_num, year_and_mintmark, mintage = tr.text.gsub("\r\n",'').strip.split(/\s\s+/).first(3)
      ...
      Coin.create!(pcgs_num: pcgs_num.to_i, year: year.to_i, mintmark: mintmark, mintage: mintage.to_i, category: category, designer: designer, denomination: denomination)
    end
  end
end
